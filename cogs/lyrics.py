...
def generate_embed(
    title: str, track_url: str,
    album_art: str, lyricsList: list,
    avatar: str, pgno: int
    ) -> Embed:
    embed = Embed(title = f'{title}',
                  url=track_url, color=0x1DB954,
                  description=lyricsList[pgno])
...
class Pages(disnake.ui.View):
    ...
    async def prev_page(self, button: Button, interaction: Interaction) -> None:
        ...
        await interaction.response.edit_message(
            embed=generate_embed(title=self.song.title,
    ...
    async def lyrics(
        ...
        if song is None:
            ...
            await inter.edit_original_message(
                embed = generate_embed(title=song.title,
                                       track_url=song.url,

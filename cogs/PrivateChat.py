...
class PrivateChat(commands.Cog):
	...
	async def create(self, inter: ApplicationCommandInteraction) -> None:
		...
		for category in inter.guild.categories:
			if category.name == f"{inter.author.display_name}'s Chat" and isinstance(inter.author, Member):
				for channel in category.channels: await channel.set_permissions(inter.author, overwrite=overwrites_readEnable)
				await category.set_permissions(inter.author, overwrite=overwrites_readEnable)
				return await inter.response.send_message(f"Created a new Private Chat ({category.mention}).", ephemeral=True)
		daBotsRole = find(lambda r: r.id == 821762347659165727, inter.guild.roles)
		overwrites_readTrue = { inter.guild.default_role: disnake.PermissionOverwrite(read_messages=False),
		...
		category: disnake.CategoryChannel = await inter.guild.create_category(f"{inter.author.display_name}'s Chat", overwrites=overwrites_readTrue, reason=None, position=None)
	...
	async def delete(self, inter: ApplicationCommandInteraction) -> None:
		...
		if isinstance(category, disnake.CategoryChannel) and isinstance(inter.author, Member):
			...
			return await inter.response.send_message("Private Chat Deleted.", ephemeral=True)
		elif category is None:
			return await inter.response.send_message("You don't have a Private Chat", ephemeral=True)
